...
class NginxGenerator {
    static async removeSiteConfigsListeningOnPort(port, keepConfigName = null) {
        ...
        // Look for "listen 3321;" or "listen 0.0.0.0:3321;"
        try {
            const { stdout } = await execAsync(`sudo grep -E -R "listen (0\\.0\\.0\\.0:)?${p};" -n ${SITES_AVAILABLE} 2>/dev/null || true`);
    ...
    # FIX: Break infinity loop if index.html is missing (Double Check)
    location = /index.html {
        try_files $uri =404;
    }
