    ...
    static async deploy(appName, zipFilePath, owner) {
        ...
        try {
            ...
            console.log(`[${appName}] Finalizing build files...`);
             // Ensure parent directory exists and has traversal/write permissions for Nginx & Node
             // We use 777 or 755 to ensure we can write into it.
            try {
                if (!fs.existsSync(BUILD_ROOT)) {
                    fs.mkdirSync(BUILD_ROOT, { recursive: true });
                }
                // Try to relax permissions on the BUILD_ROOT so we can rename into it
                await execAsync(`sudo chmod 777 ${BUILD_ROOT}`);
            } catch (e) { console.warn('Warning: Could not set permissions on build root:', e.message); }
