...
class Deployer {
    static async deploy(appName, zipFilePath, owner) {
        ...
        try {
            ...
            // Only 'execute' (+x) is needed on parents for traversal.
            /* 
               If the path is /home/ubuntu/actions-runner/.../builds
               We need +x on: /home, /home/ubuntu, ... /builds
            */
            try {
                ...
                while (currentDir !== fsRoot) {
                    ...
                    } catch (e) { /* Ignore errors on folders we don't own/can't touch */ }
                    currentDir = path.dirname(currentDir);
                }
                
                // Also ensure 777 on the build root itself so we can write into it
                await execAsync(`sudo chmod 777 "${BUILD_ROOT}"`);

            } catch (e) { console.warn('Warning: Could not set recursive traversal permissions:', e.message); }
            ...
            // Revert Port Allocation in ports.json so reconcile doesn't try to revive it
            try {
                if (previousPortMeta) {
                ...
                console.warn(`[${appName}] Failed to revert port allocation:`, releaseErr.message);
            }
    ...
    static async deployDocker(appName, imageName, owner, internalPort = 80, category = 'backend') {
        ...
        try {
            ...
            // RELOAD NGINX NOW to actually release the port!
            await NginxGenerator.reloadNginx();
            ...
            for (let attempt = 1; attempt <= maxPortAttempts; attempt++) {
                ...
                console.log(`[${appName}] Starting container (attempt ${attempt}/${maxPortAttempts}): ${imageName} on port 0.0.0.0:${allocatedPort}:${internalPort}...`);

                // Use -d for detached, --restart always for persistence, with resource limits
                // Force bind to IPv4 (0.0.0.0) to avoid IPv6 errors
                const dockerCmd = `sudo docker run -d --name ${appName} -p 0.0.0.0:${allocatedPort}:${internalPort} --restart ${DOCKER_RESTART_POLICY} --memory ${DOCKER_MEMORY_LIMIT} --cpus ${DOCKER_CPU_LIMIT} ${imageName}`;
                ...
                try {
                    ...
                    // Comprehensive check for all flavors of "Port Busy" errors
                    const isPortInUse = details.includes('address already in use') || 
                                      details.includes('failed to bind host port') || 
                                      details.includes('Bind for') || 
                                      details.includes('port is already allocated') ||
                                      details.includes('driver failed programming external connectivity') ||
                                      details.includes('listen tcp 0.0.0.0:') ||
                                      details.includes('bind: address already in use');

                    console.warn(`[${appName}] Container start failed (Attempt ${attempt}): ${e.message}`);
                    ...
                    if (!isPortInUse || attempt === maxPortAttempts) {
                        if (isPortInUse) console.error(`[${appName}] Exhausted all port retries.`);
                        throw e;
                    }

                    console.warn(`[${appName}] Port ${allocatedPort} is busy. Auto-healing: requesting new port...`);
                    
                    // Mark this port as bad so we don't pick it again
                    attemptedPorts.add(allocatedPort);
                    ...
                    // Cleanup potential conflict on new port just in case (e.g. Nginx binding)
                    await NginxGenerator.removeSiteConfigsListeningOnPort(allocatedPort);
                    try { await NginxGenerator.reloadNginx(); } catch(e) {}
                    
                    console.log(`[${appName}] Retrying with new port: ${allocatedPort}`);
                }
            }
    ...
    static async getContainerResourceStats(appNames) {
        if (!appNames || appNames.length === 0) return {};
        try {
            const { stdout } = await execAsync("sudo docker stats --no-stream --format '{{.Name}}|{{.CPUPerc}}|{{.MemUsage}}|{{.NetIO}}|{{.BlockIO}}'");
            const stats = {};

            stdout.split('\n').map(line => line.trim()).filter(Boolean).forEach(line => {
                const parts = line.split('|');
                if (parts.length < 3) return;
                const [name, cpu, mem, netIO, blockIO] = parts;
            ...
            console.warn('Failed to get docker resource stats:', e.message);
            return {};
        }
    }
}
