    ...
    static async deploy(appName, zipFilePath, owner) {
        ...
        try {
            ...
            console.log(`[${appName}] Finalizing build files...`);
             // Ensure parent directory exists and has traversal permissions for Nginx
            try {
                if (!fs.existsSync(BUILD_ROOT)) fs.mkdirSync(BUILD_ROOT, { recursive: true });
                await execAsync(`chmod +x ${path.dirname(BUILD_ROOT)}`);
                await execAsync(`chmod +x ${BUILD_ROOT}`);
            } catch (e) { console.warn('Warning: Could not set permissions on build root parents:', e.message); }
